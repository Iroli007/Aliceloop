...
- 为 fetch / browser / travel / send-file / review 之类高层能力继续扩展新的原生 agent-loop tool
...
- `travel` 是一个 skill。它的能力应当建立在 `read`、`grep`、`glob`、`write`、`edit`、`bash` 这些底层原子命令之上，再组合其他 skill，而不是继续往 agent loop 底层塞新的 native tool。
