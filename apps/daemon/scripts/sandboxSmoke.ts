...
async function main() {
  ...
  const [
    ...
    import("../src/context/tools/bashSandboxDecision.ts"),
  ]);
  ...
  const manualDeleteResult = await cleanupSandbox.runBash({
    command: "rm",
    args: [manualFilePath],
    cwd: tempDataDir,
  });
  assert(manualDeleteResult.stdout.includes("deleted:"), "rm via bash should report deleted manual files");
  assert.equal(existsSync(manualFilePath), false, "rm via bash should remove manual files inside the sandbox roots");
  ...
  const fullAccessWorkspaceSandbox = createPermissionSandboxExecutor({
    label: "sandbox-smoke-full-access-workspace-boundary",
    permissionProfile: "full-access",
  ...
  const fullAccessWorkspacePolicy = fullAccessWorkspaceSandbox.describePolicy();
  assert.equal(fullAccessWorkspacePolicy.allowedReadRoots.includes(tempDataDir), true, "full-access workspace boundary should keep explicit read roots");
  assert.equal(fullAccessWorkspacePolicy.allowedWriteRoots.includes(tempDataDir), true, "full-access workspace boundary should keep explicit write roots");
  assert.equal(fullAccessWorkspacePolicy.allowedCwdRoots.includes(tempDataDir), true, "full-access workspace boundary should keep explicit cwd roots");
  const fullAccessFallbackScriptPath = join(externalDir, `fallback-script-${Date.now()}.txt`);
  assert.equal(
    decideAgentBashExecutionMode(
      {
        command: "cat",
        args: [join(tempDataDir, "artifacts", "note.txt")],
        cwd: tempDataDir,
      },
      {
        workspaceRoot: tempDataDir,
        defaultCwd: tempDataDir,
      },
    ),
    "sandbox",
    "workspace-local bash should stay on the sandbox route",
  );
  assert.equal(
    decideAgentBashExecutionMode(
      {
    ...
    "host",
    "bash that reads outside the workspace should route directly to host execution",
  );
  assert.equal(
    decideAgentBashExecutionMode(
      {
        command: "",
        script: `printf 'fallback-ok' > ${fullAccessFallbackScriptPath} && cat ${fullAccessFallbackScriptPath}`,
        cwd: tempDataDir,
      },
      {
        workspaceRoot: tempDataDir,
    ...
    "host",
    "script redirects outside the workspace should route directly to host execution",
  );
  assert.equal(
    decideAgentBashExecutionMode(
      {
    ...
    "sandbox",
    "complex bash syntax should stay on the sandbox path and fail closed",
  );

  const fullAccessFallbackRead = await fullAccessWorkspaceSandbox.runBash({
    command: "cat",
    args: [sourcePath],
    cwd: tempDataDir,
  });
  assert.equal(fullAccessFallbackRead.stdout, "alpha\nbeta\n", "full-access workspace bash should retry unsandboxed once for paths outside the workspace roots");

  const fullAccessFallbackScript = await fullAccessWorkspaceSandbox.runBash({
    command: "",
    script: `printf 'fallback-ok' > ${fullAccessFallbackScriptPath} && cat ${fullAccessFallbackScriptPath}`,
    cwd: tempDataDir,
  });
  assert.equal(fullAccessFallbackScript.stdout.trim(), "fallback-ok", "full-access workspace script should retry unsandboxed once when a redirect leaves the workspace roots");
  assert.equal(readFileSync(fullAccessFallbackScriptPath, "utf8"), "fallback-ok");

  writeFileSync(manualFilePath, "manual-again\n", "utf8");
