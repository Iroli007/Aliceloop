...
export function needsWebResearch(query: string) {
  return matches(
    query,
    /^search$|^web[\s_-]?search$|^websearch$|查|找一下|帮我找|找人|找这个|找这个人|找这个ID|搜|搜索|research|fact-?check|验证|核对|确认|准确|准不准|可靠吗|可靠性|事实依据|来源|source|最新|当前|最近|今天|日期|几月几日|天气|温度|粉丝|关注者|播放|点赞|价格|汇率|比分|政策|新闻|官网|网址|链接|url|https?:\/\/|网上.*(好玩|有意思|新鲜事)|互联网.*(好玩|有意思)|上网.*看看|搜搜看/iu,
  );
}
