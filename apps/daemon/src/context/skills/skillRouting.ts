...
export function needsWebResearch(query: string) {
  ...
  return matches(
    query,
    /^search$|^web[\s_-]?search$|^websearch$|搜索|搜一下|搜一搜|搜搜看|research|调查|fact-?check|验证|核对|确认|准确|准不准|可靠吗|可靠性|事实依据|来源|source|天气|温度|粉丝|关注者|播放|点赞|价格|汇率|比分|政策|新闻|官网|网址|链接|url|https?:\/\/|(?:现在|当前|目前|当下|近年|这几年).{0,16}(?:常见|主流|流行|热门|趋势|现状|分类|范式|方案|做法|发展|有哪些|有什么)|网上.*(好玩|有意思|新鲜事)|互联网.*(好玩|有意思)|上网.*看看|搜搜看/iu,
  );
}
