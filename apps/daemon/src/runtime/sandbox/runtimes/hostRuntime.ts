...
async function runBash(context: SandboxRuntimeContext, input: RunBashInput) {
  ...
  const hasFilesystemBoundary = context.toolPolicy.allowedReadRoots !== null
    || context.toolPolicy.allowedWriteRoots !== null
    || context.toolPolicy.allowedCwdRoots !== null;
  const scriptCommands = (() => {
    ...
    if (!context.toolPolicy.fullAccess) {
      return parseShellScriptCommandsForPolicy(script);
    }
    ...
    try {
      ...
      if (error instanceof SandboxViolationError && !error.allowElevatedFallback) {
        return [];
      }
      throw error;
    }
