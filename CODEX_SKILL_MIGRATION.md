...
### 当前不直接照搬的 Skills（改为 Alice 原生版本或暂不保留）

| Skill | 原因 |
|-------|------|
| `selfie` | 不直接照搬 Alma 版本；当前保留为 Alice 原生 `planned` skill，等待 reference-image 能力接入 |
| `travel` | 虚拟旅行系统，娱乐功能 |
| `music-listener` | 已改造为 `audio-analysis` |
| `video-reader` | 已改造为 `video-analysis`，不再依赖 `alma video` |
| `xiaohongshu-cli` | 已改造为 `xiaohongshu`，使用直接 `xhs` CLI |
| `twitter-media` | 保留名称，但内容已改成 Alice 本地工作流 |
| `skill-search` | 已改造为 `skill-discovery`，不再依赖 skills.sh 生态 |

### 迁移现状补充

当前 repo 中与这批 refactor 相关的 skill 状态：

| 当前 Skill | status | 说明 |
|-----------|--------|------|
| `skill-discovery` | available | 本地 catalog 发现与推荐 |
| `audio-analysis` | available | 音频元数据 / 频谱 / 转录分析 |
| `video-analysis` | available | 视频抽帧 + 音频转录分析 |
| `twitter-media` | available | fxtwitter-compatible 抽取 |
| `xiaohongshu` | available | 直接 `xhs` CLI 工作流 |
| `selfie` | planned | 目录与 album 设计已落位，等待 reference-image 后端 |
| `travel` | removed | 已从当前 skill catalog 移除 |
